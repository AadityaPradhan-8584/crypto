...
def train_model_for_coin(coin_symbol):
    print(f"\n--- Starting Multivariate Process for {coin_symbol} ---")
    ...
    print(f"Training multivariate model for {coin_symbol}...")
    history = model.fit(
        X_train, y_train,
    ...
    plt.plot(close_price_actual[:, 0], color='blue', label='Actual Price (First Hour of Forecast)')
    plt.plot(close_price_pred[:, 0], color='red', label='Predicted Price (First Hour of Forecast)', alpha=0.7)
    plt.title(f'{coin_symbol}-INR Close Price Prediction (Test Set)')
